...
class Neo4jGraphStore(GraphStore):
    ...
    def get(self, subj: str) -> List[List[str]]:
        """Get triplets."""
        logger.debug(f"nodel_label:\n{self.node_label}")
        # query = """
        #     MATCH (n1:%s)-[r]->(n2:%s)
        #     WHERE n1.name = $subj
        #     RETURN type(r), n2.name;
        # """

        # prepared_statement = query % (self.node_label, self.node_label)
        query = """
            MATCH (n1)-[r]->(n2)
            WHERE n1.name = $subj OR n2.name = $subj
            RETURN type(r), n2.name;
        """
    ...
    def get_rel_map(
        ...
        #     f"""{"WHERE n1.name IN $subjs" if subjs else ""} """
        #     "UNWIND relationships(p) AS rel "
        #     "WITH n1.name AS subj, p, apoc.coll.flatten(apoc.coll.toSet("
        #     "collect([type(rel), endNode(rel).name]))) AS flattened_rels "
        #     f"RETURN subj, collect(flattened_rels) AS flattened_rels LIMIT {limit}"
        # )
        query = (
            f"""MATCH p=(n1)-[*1..{depth}]->() """
            f"""{"WHERE n1.name IN $subjs" if subjs else ""} """
            "UNWIND relationships(p) AS rel "
            "WITH n1.name AS subj, p, apoc.coll.flatten(apoc.coll.toSet("
            "collect([type(rel), endNode(rel).name]))) AS flattened_rels "
        ...
        if not data:
            reverse_query = (
                ...
                "UNWIND relationships(p) AS rel "
                "WITH n1.name AS subj, p, apoc.coll.flatten(apoc.coll.toSet("
                "collect([startNode(rel).name, type(rel), endNode(rel).name]))) AS flattened_rels "
                f"RETURN subj, collect(flattened_rels) AS flattened_rels LIMIT {limit}"
            )
            reverse_data = list(self.query(reverse_query, {"subjs": subjs}))
            if not reverse_data:
                return rel_map
            else:
    ...
    def delete(self, subj: str, rel: str, obj: str) -> None:
        ...
        def delete_rel(subj: str, obj: str, rel: str) -> None:
            with self._driver.session(database=self._database) as session:
                session.run(
                    (
                        "MATCH (n1:{})-[r:{}]->(n2:{}) WHERE n1.name = $subj AND n2.name"
                        " = $obj DELETE r"
        ...
        def delete_entity(entity: str) -> None:
            with self._driver.session(database=self._database) as session:
                session.run(
                    "MATCH (n:%s) WHERE n.name = $entity DELETE n" % self.node_label,
                    {"entity": entity},
                )
        ...
        def check_edges(entity: str) -> bool:
            with self._driver.session(database=self._database) as session:
                is_exists_result = session.run(
                    "MATCH (n1:%s)--() WHERE n1.name = $entity RETURN count(*)"
                    % (self.node_label),
                    {"entity": entity},
                )
